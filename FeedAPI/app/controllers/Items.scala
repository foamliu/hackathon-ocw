...
case class Item(itemID: Long, var title: String, description: String, piclink: String, courselink: String, duration: String, source: String, school: String, instructor: String, language: String, tags: String, link: String, enabled: Boolean, posted: String)
...
object Item {
    ...
    implicit val itemReads: Reads[Item] = (
        ...
        (JsPath \\ "posted").read[String])(Item.apply _)

    implicit val itemWrites = new Writes[Item] {
        def writes(c: Item): JsValue =
            Json.obj(
                "item_id" -> c.itemID,
                "title" -> c.title,
